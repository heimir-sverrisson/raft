...
struct RaftProtocol_ : public msm::front::state_machine_def<RaftProtocol_> {
    ...
    struct Candidate_ : public msm::front::state_machine_def<Candidate_> {
        ...
        struct ExitToFollower
            : public msm::front::exit_pseudo_state<GotRequestVote> {
            template <class Event, class Fsm>
            void on_entry(Event const&, Fsm& fsm) const {
        ...
        struct ExitToLeader
            : public msm::front::exit_pseudo_state<GotVoteResponse> {
            template <class Event, class Fsm>
            void on_entry(Event const&, Fsm& fsm) const {
        ...
        void sendMyVote(const GotRequestVote& evt) {
            ...
            if (hisTerm > myTerm) {
                BOOST_LOG_TRIVIAL(info)
                    << "Voting as candidate for: " << candidateId;
                s.sendVoteResponse(*ssp_, candidateId, 1);  // He gets our vote
            } else {
                BOOST_LOG_TRIVIAL(info)
                    << "Not voting as candidate for: " << candidateId;
        ...
        struct transition_table
            : mpl::vector<
                  //     Start,               Event,           Next,
                  //     Action,                 Guard
                  a_row<SetRandomTimeout, Timeout, WaitForVoteResponse,
                        &cd::startElections>,
                  row<SetRandomTimeout, GotRequestVote, ExitToFollower,
                      &cd::sendMyVote, &cd::isHisTermHigher>,
                  _row<WaitForVoteResponse, Timeout, SetRandomTimeout>,
                  row<WaitForVoteResponse, GotRequestVote, ExitToFollower,
                      &cd::sendMyVote, &cd::isHisTermHigher>,
                  row<WaitForVoteResponse, GotVoteResponse, ExitToLeader,
                      &cd::takeLeadership, &cd::gotEnoughVotes> > {};
    ...
    void giveUpLeadership(const GotAppendEntries& evt) {
        ...
        BOOST_LOG_TRIVIAL(error) << "Somebody send me AppendEntries - but I "
                                    "thought I was the leader";
    ...
    void stopElections(const GotAppendEntries& evt) {
        ...
        BOOST_LOG_TRIVIAL(error)
            << "Somebody sent me AppendEntries - aborting elections";
    }
    ...
    void processAppendEntries(const GotAppendEntries& evt) {
        ...
        if (myTerm != hisTerm) {
            BOOST_LOG_TRIVIAL(error) << "myTerm: " << myTerm
                                     << ", hisTerm: " << hisTerm;
            if (myTerm == 0) {
        ...
        Sender s;
        s.sendAppendResponse(ss_, myLeader,
                             1);  // ToDO: compute success correctly
    }

    void processAppendResponse(const GotAppendResponse& evt) {
        BOOST_LOG_TRIVIAL(info) << "AppendResponse: " << evt.ar_.to_string();
        // TODO: Move the state of an entry to committed when majority is
        // reached
    }
    ...
    struct transition_table
        : mpl::vector<
              ...
              a_row<Follower, GotAppendEntries, Follower,
                    &rp::processAppendEntries>,
              row<Candidate, GotAppendEntries, Follower, &rp::stopElections,
                  &rp::isHisTermHigherOrSame>,
              row<Candidate, GotRequestVote, Follower, &rp::sendMyVote,
                  &rp::isHisTermHigherOrSame>,
              row<Leader, GotAppendEntries, Follower, &rp::giveUpLeadership,
                  &rp::isHisTermHigher>,
              row<Leader, GotRequestVote, Follower, &rp::sendMyVote,
                  &rp::isHisTermHigher>,
              a_row<Leader, Timeout, Leader, &rp::sendHeartbeat>,
              a_row<Leader, GotAppendResponse, Leader,
                    &rp::processAppendResponse>,
              _row<Candidate::exit_pt<Candidate_::ExitToFollower>,
                   GotRequestVote, Follower>,
              _row<Candidate::exit_pt<Candidate_::ExitToLeader>,
                   GotVoteResponse, Leader> > {};
    ...
    inline void resetTimeout() {
        ...
        using namespace std::chrono;
        until_ = duration_cast<milliseconds>(
                     high_resolution_clock::now().time_since_epoch()).count() -
                 3;
    }
