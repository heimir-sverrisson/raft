...
void Sender::sendVoteResponse(ServerState& ss, int candidateId, int vote) {
...
void Sender::sendAppendResponse(ServerState& ss, int leaderId, int success) {
    char sep = Config::messageSeparator;
...
void Sender::sendRequestVote(ServerState& ss) {
    int myId = ss.getMyId();
    char sep = Config::messageSeparator;
    RequestVote rv(ss.getTerm(), myId, ss.getLastLogIndex(),
                   ss.getLastLogTerm());
...
void Sender::sendAppendEntries(ServerState& ss) {
    char sep = Config::messageSeparator;
    BOOST_LOG_TRIVIAL(info) << "Sender: Sending AppendEntries to all";
    AppendEntries ae(ss.getTerm(), ss.getMyId(), 1, 2,
                     3);  // TODO: correct params needed
    ...
    BOOST_LOG_TRIVIAL(info) << "Sender: All AppendEntries sent";
}
